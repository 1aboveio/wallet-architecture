...
6. 保证金比率与 MDR 独立配置，不做交叉校验（见 ADR 0003）
7. 扣除顺序：MDR → 固定保证金 → 滚动保证金
...
```
原单 $100, MDR 1%, 固定保证金 3%, 滚动保证金 5%

  MDR = $100 × 1% = $1.00
  固定保证金 = $100 × 3% = $3.00
  滚动保证金 = $100 × 5% = $5.00
  商户实收 = $100 - $1 - $3 - $5 = $91.00
...
```
原单 $100, MDR 10%, 固定保证金 95%, 滚动保证金 5%

  MDR = $100 × 10% = $10.00
  剩余 = $100 - $10 = $90
  固定保证金应扣 = $100 × 95% = $95
  固定保证金实际 = min($95, $90) = $90（封顶）
  滚动保证金 = $100 × 5% = $5.00
  商户实收 = $100 - $10 - $90 - $5 = -$5.00（负余额）

  借  customer:abc:pending:USD          -$100.00
  贷  customer:abc:available:USD        -$5.00     ← 负余额
  贷  customer:abc:reserve:fixed:USD    +$90.00
  贷  customer:abc:reserve:rolling:USD  +$5.00
