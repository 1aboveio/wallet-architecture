...
上游（收单行 → Glopay）与下游（Glopay → 商户）是**两本独立的账**，互不影响：

- **上游**：收单行结算时间、到账金额、通道费 —— Glopay 与收单行之间
- **下游**：基于原始交易金额，按约定费率和保证金规则给商户结算 —— Glopay 与商户之间

```
上游账本（Glopay ↔ 收单行）          下游账本（Glopay ↔ 商户）
┌──────────────────────┐           ┌──────────────────────┐
│ 收单行结算: $97.50    │           │ 原交易金额: $100      │
│ 通道费: $2.50         │           │ 退款: -$30            │
│                      │    独立   │ 服务费 1%×$100: -$1   │
│ Glopay 银行到账       │◄─ ─ ─ ─►│ 保证金 5%×$70: -$3.50 │
│                      │           │                      │
│ Glopay 的成本         │           │ 商户实收: $65.50      │
│ 影响 Glopay 利润      │           │                      │
└──────────────────────┘           └──────────────────────┘
```
